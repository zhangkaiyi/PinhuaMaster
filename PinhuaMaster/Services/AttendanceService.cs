...
namespace PinhuaMaster.Services
{
    public class AttendanceService : IAttendanceService
    {
        ...
        public IList<AttendanceTimeRecords> GetTimeRecrods(int Y, int M)
        {
            ...
            // 正常打卡
            var records1 = (from f in _pinhuaContext.人员档案.ToList()
                            join c in _pinhuaContext.考勤卡号变动.ToList() on f.ExcelServerRcid equals c.ExcelServerRcid
        ...
        {
            var records = GetTimeRecrods(Y, M).OrderBy(p => p.Id).ThenBy(p => p.SignTime);
            ...
            foreach (var record in recordsGroupBy)
            {
                ...
                for (int i = 0; i != days; i++)
                {
                    ...
                    foreach (var range in rule.Ranges)
                    {
                        (var workfrom, var workto) = 指定日期的班段起止(range, new DateTime(Y, M, D));

                        (var clockinfrom, var clockinto) = 指定日期的签到起止(range, new DateTime(Y, M, D));
                        ...
                        (var clockoutfrom, var clockoutto) = 指定日期的签退起止(range, new DateTime(Y, M, D));
                        ...
                        if (checkin.HasValue && checkout.HasValue)
                        {
                            ...
                            if (range.中途休息 == 1)
                            {
                                var restfrom = range.休息开始时间.Value.ConvertToTargetDate(new DateTime(Y, M, D));
                                var restto = range.休息结束时间.Value.ConvertToTargetDate(new DateTime(Y, M, D));
