...
namespace PinhuaMaster.Pages.Retail.Delivery
{
    public class DetailsModel : PageModel
    {
        ...
        public void OnGet(string Id)
        {
            ...
            var remoteOrder = _pinhuaContext.RetailGiMain.AsNoTracking().FirstOrDefault(p => p.DeliveryId == Id);
            if (remoteOrder == null)
                return;

            Order = new Gi2ViewModel
            {
                Main = _mapper.Map<RetailGiMain, Gi2MainDTO>(remoteOrder),
                Details = _mapper.Map<List<RetailGiDetails>, List<Gi2DetaislDTO>>(_pinhuaContext.RetailGiDetails.AsNoTracking().Where(p => p.DeliveryId == remoteOrder.DeliveryId).ToList()),
            };
